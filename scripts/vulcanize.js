...
if (!sourceFile) throw new Error(`Error: couldn't find HTML file in ${bowerDir + entryFile}`);

vulcan.process(`${sourceDir}/${sourceFile}`, function(err, inlinedHtml) {
  if (err) throw new Error(`Error: couldn't vulcanise ${sourceDir + sourceFile}. ${err}`);
  else fs.outputFileSync(`${distDir}/${entryFile}/${entryFile}.html`, inlinedHtml);
